# Code for setting 3 for linear regression
## numerical results
## plots
...
for (q in q_set){
  for (sig_delta2 in sig_delta2_set){
    ...
    OUTPUT_DIR <- here("Linear", "simu", "res", "setting 3")
...
for (ii in 1:nrow(params_all)){
  ...
  loc <- here("Linear", "simu", "res", "setting 3")
...
data_measure_setting3$sig_delta2 <- factor(data_measure_setting3$sig_delta2, 
                                           ...
                                           labels = c("Linear: rho=0.3","Linear: rho=0.5","Linear: rho=1","Linear: rho=1.5"))
data_measure_setting3$Method <- factor(data_measure_setting3$Method, levels = Method)

p1_Gaussian <- ggplot(data_measure_setting3, aes(x = q, y=loc_rmse, color = Method))+
...
ggsave(p1_Gaussian, filename=paste0(OUTPUT_DIR, "/setting", 3, "-est.pdf",collapse=""), width=5.6, height=2.2)
...
ggsave(p1_Gaussian, filename=paste0(OUTPUT_DIR, "/setting", 3, "-pred.pdf",collapse=""), width=5.6, height=2.2)
...
p1_Gaussian_fdr <- ggplot(data_measure_setting3, aes(x = q, y=loc_rmse, color = Method))+
  geom_line(aes(linetype=Method, color=Method), size = 0.7)+
  geom_point(aes(color=Method, shape=Method), size = 1.3)+
  geom_errorbar(aes(ymin=loc_rmse-loc_se,
                    ymax=loc_rmse+loc_se),
...
ggsave(p1, filename=paste0(OUTPUT_DIR, "/setting", 3, "-tpr.pdf",collapse=""), width=7.2, height=3.6)
