# Code for setting 2 for logistic regression
## numerical results
## plots
...
for (s in s_set){
  for (ratio in ratio_set){
    ...
    OUTPUT_DIR <- here("Binary", "simu", "res", "setting 2")
...
for (ii in 1:nrow(params_all)){
  ...
  loc <- here("Binary", "simu", "res", "setting 2")
  loc_res <- read.csv(paste(loc, "/Res setting 2 s ", s, " ratio ", ratio, " sig beta 1.csv", sep=""))
...
data_measure_setting2$s <- factor(data_measure_setting2$s,
                                  ...
                                  labels = c("Logistic: s=4","Logistic: s=8","Logistic: s=12","Logistic: s=16"))
data_measure_setting2$Method <- factor(data_measure_setting2$Method, levels = Method)

p1_logistic <- ggplot(data_measure_setting2, aes(x = ratio, y=loc_rmse, color = Method))+
...
ggsave(p1, filename=paste0(OUTPUT_DIR, "/setting", 2, "-est.png",collapse=""), width=4.2, height=2.4)
...
ggsave(p1, filename=paste0(OUTPUT_DIR, "/setting", 2, "-pred.png",collapse=""), width=4.2, height=2.4)
...
p1_logistic_fdr <- ggplot(data_measure_setting2, aes(x = ratio, y=loc_rmse, color = Method))+
  geom_line(aes(linetype=Method, color=Method), size = 0.7)+
  geom_point(aes(color=Method, shape=Method), size = 1.3)+
  geom_errorbar(aes(ymin=loc_rmse-loc_se,
                    ymax=loc_rmse+loc_se),
...
ggsave(p1, filename=paste0(OUTPUT_DIR, "/setting", 2, "-tpr.pdf",collapse=""), width=5.6, height=3.6)
